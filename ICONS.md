...
the shared set below — **never a colored emoji** (🌙 ☀️ ♈ …). Emoji render differently on every
...
recursive.eco, and this repo's own `index.html` / viewers.
...
HTML page or README here (this repo has no React). If you only work here, this file is enough.

> **Primary glyph for this repo: Astrology** — a crescent moon + a single star (celestial, *not* a
> magic sparkle). It's the same mark the Astrology oracle and the recursive.eco channel hero use.
> (There's no galaxy/black-hole glyph in the shared set yet — the crescent-moon-+-star Astrology
> mark stands in for the whole sky family.) Never label a sky surface with 🌌 / 🪐 / ✨.

## How to use

Every icon shares one frame: `viewBox="0 0 24 24"`, `fill="none"`, `stroke="currentColor"`,
`stroke-linecap="round"`, `stroke-linejoin="round"`, stroke width **1.75**. **Color** = whatever text
color the parent sets (`color:` / a class — Kali·Paradevi's is `var(--paraink)` or `var(--kali)`).
**Size** = set `width`/`height`.

```html
<span style="color:var(--kali)">
...
### Astrology — crescent moon + star  *(this repo's primary glyph)*
```html
...
### Tarot — card + lozenge  *(the sibling divination repo, recursive-tarot)*
```html
...
## Never do this
- ❌ `<h1>🌌 Kali · Paradevi</h1>`
- ✅ Astrology SVG (above) next to `<h1>Kali · Paradevi</h1>`
